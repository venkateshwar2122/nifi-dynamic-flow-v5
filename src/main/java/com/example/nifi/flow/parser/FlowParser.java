package com.example.nifi.flow.parser;
...
public class FlowParser {
    ...
    public FlowContext parse(FlowRequest request) {
        ...
        try {
            ...
            // =========================
            log.info("[{}] Step 5: Extracting tables from source schema", traceId);
            ...
            List<FlowTable> tables = new ArrayList<>();

            for (Map<String, Object> schemaItem : schema) {
                List<Map<String, Object>> schemaTables =
                        (List<Map<String, Object>>) schemaItem.get("tables");
                ...
                for (Map<String, Object> table : schemaTables) {
                    ...
                    validateIdColumn(table, tableName);

                    String collectionName = tableName;
                    tables.add(new FlowTable(tableName, collectionName));
                    log.info("[{}] Table selected: {}", traceId, tableName);
                }
            }

            if (tables.isEmpty()) {
                throw new IllegalArgumentException("No tables found in source schema");
            }

            ctx.setTables(tables);
            ctx.setTableName(tables.get(0).getTableName());
            ctx.setCollectionName(tables.get(0).getCollectionName());

            log.info("[{}] Tables selected for deployment: {}", traceId, tables.size());
    ...
    private void validateIdColumn(Map<String, Object> table, String tableName) {
